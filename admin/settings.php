...
try {
    $stmt = $pdo->query("SELECT `key`, `value` FROM settings WHERE `key` IN ('store_name', 'telegram_bot_token', 'telegram_chat_id')");
    $settings = [];
...
// settings.php (в разделе POST-обработчиков)
...
<head>
    ...
    .admin-wrapper {
        ...
        min-height: 100vh;
        overflow-y: auto;
        background-color: #f8f9fa; /* Фоновый цвет для всего макета */
    }
    ...
    .admin-main {
        flex: 1;
        padding: 20px;
        margin-left: 250px; /* Сдвиг для десктопной версии */
        padding-top: 20px;
    }
...
<body class="admin-wrapper">
    ...
    <div class="admin-content">
        <div class="container-fluid p-3 p-md-4">
            ...
            <!-- settings.php (после настроек магазина) -->
            <div class="card admin-form mb-4">
                <div class="card-body p-3 p-md-4">
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <nav aria-label="Page navigation">
                        <ul class="pagination justify-content-center" id="activityLogPagination"></ul>
                    </nav>
...
window.loadActivityLog = async function(page = 1) {
    console.log('Loading activity log for page:', page);
    ...
    try {
        ...
        renderActivityLog(data.logs, data.total_pages, page);
        
    } catch (error) {
        console.error('Ошибка:', error);
        contentDiv.innerHTML = `
            <div class="alert alert-danger">
                <h5>Ошибка при загрузке журнала действий</h5>
                <p>${error.message}</p>
                <button class="btn btn-sm btn-outline-primary" onclick="loadActivityLog(${page})">
...
window.renderActivityLog = function(logs, totalPages, currentPage) {
    const contentDiv = document.getElementById('activity-log-content');
    const paginationDiv = document.getElementById('activityLogPagination');
    const overlay = document.querySelector('.overlay');
    const body = document.body;
    ...
    if (!logs || logs.length === 0) {
        contentDiv.innerHTML = '<div class="alert alert-info">Нет записей в журнале действий</div>';
    ...
    if (startPage > 1) {
        paginationHtml += `<li class="page-item"><a class="page-link" href="#" onclick="loadActivityLog(1); return false;">1</a></li>`;
    ...
    for (let i = startPage; i <= endPage; i++) {
        paginationHtml += `
            <li class="page-item ${i === currentPage ? 'active' : ''}">
                <a class="page-link" href="#" onclick="loadActivityLog(${i}); return false;">${i}</a>
            </li>`;
    }

    if (endPage < totalPages) {
        if (endPage < totalPages - 1) paginationHtml += `<li class="page-item disabled"><span class="page-link">...</span></li>`;
        paginationHtml += `<li class="page-item"><a class="page-link" href="#" onclick="loadActivityLog(${totalPages}); return false;">${totalPages}</a></li>`;
    }
