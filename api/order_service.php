...
if ($delivery_company === 'cdek') {
    if (!function_exists('curl_init')) {
        ...
        if (!$token) {
            ...
            // Извлечение кода ПВЗ из pickup_point
            $delivery_point_code = explode(',', $pickup_point)[0] ?? $pickup_city;
            $orderData = [
                'number' => $order_number,
                'tariff_code' => '1',
                'recipient' => [
...
logToFile("Вызов sendTelegramOrderNotification с order_id: $order_id", "DEBUG");
$result = sendTelegramOrderNotification($pdo, $order_id);
if ($result) {
    error_log("Уведомление о заказе $order_id отправлено успешно");
} else {
    error_log("Не удалось отправить уведомление о заказе $order_id");
}
